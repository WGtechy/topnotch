...
import { componentLoader } from "../../bucket/loading-components/componentLoader";
  const AllCompletedOrders = (props) => {
    const {
      open,
      handleClose,
    accountId,
    } = props;
    const {completedOrders: data, loadingCustomers: loading} = useSelector(state=>state.userProfile) 
        ...
        <AppBar>
          <Toolbar>
            <div onClick={handleClose} className="modalBack">
              ...
              <span className="headerLeftSection">All completed orders</span>
            </div>
          </Toolbar>
  ...
  export default memo(AllCompletedOrders);
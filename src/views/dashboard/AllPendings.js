...
import { componentLoader } from "../../bucket/loading-components/componentLoader";
  const AllPendings = (props) => {
    const {
      open,
      handleClose,
    accountId,
    } = props;
    const {pendings: data, loadingPendings: loading} = useSelector(state=>state.userProfile) 
        ...
        <AppBar>
          <Toolbar>
            <div onClick={handleClose} className="modalBack">
              ...
              <span className="headerLeftSection">All pending orders</span>
            </div>
          </Toolbar>
  ...
  export default memo(AllPendings);
...
const AllProducts = (props) => {
  ...
  useEffect(() => {
    if (product?._id && open) {
      setCurrentData((init) => [product, ...init]);
    }
  }, [product, open]);

  const cardDisplay = ({ item, i, ref }) => {
    return <div key={i} ref={ref ? lastDataRefElement : null}></div>;
  };
        ...
        <AppBar>
          <Toolbar>
            <div onClick={handleClose} className="modalBack">
              ...
              <span className="headerLeftSection">All products</span>
            </div> 
          </Toolbar>
        </AppBar>

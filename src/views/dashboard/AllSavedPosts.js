...
import { componentLoader } from "../../bucket/loading-components/componentLoader";
  const AllSavedPosts = (props) => {
    const {
      open,
      handleClose,
    accountId,
    } = props;
    const {savedPosts: data, loadingSavedPosts: loading} = useSelector(state=>state.userProfile) 
        ...
        <AppBar>
          <Toolbar>
            <div onClick={handleClose} className="modalBack">
              ...
              <span className="headerLeftSection">All saved posts</span>
            </div>
          </Toolbar>
  ...
  export default memo(AllSavedPosts);
...
import { componentLoader } from "../../bucket/loading-components/componentLoader";
  const AllSharedPosts = (props) => {
    const {
      open,
      handleClose,
    accountId,
    } = props;
    const {sharedPosts: data, loadingSharedPosts: loading} = useSelector(state=>state.userProfile) 
        ...
        <AppBar>
          <Toolbar>
            <div onClick={handleClose} className="modalBack">
              ...
              <span className="headerLeftSection">All shared products</span>
            </div>
          </Toolbar>
  ...
  export default memo(AllSharedPosts);
...
import HomePage from "./postCardComponents/HomePage";
...
const Home = (props) => {
  ...
  const arrData = useArrayData(currentData); 
   const {all,loading  } = useSelector((state) => state.posts);
  ...
  useEffect(() => {
    if (!!all?.length) {
      setCurrentData((init) => [...init, ...all]);
    } 
  }, [all]);

  useEffect(() => {
    if (accountId) {
      dispatch(getPosts({ page: postPage, accountId}));
    } else {
      dispatch(getPosts({ page: postPage }));
    }
  ...
  const cardDisplayMobile = ({ item, i, ref }) => {
    return item?.template === "promotion" ? (
      <div
        ...
        data={item._id}
      >
        <div className="postCardContainerTitle">Promotions</div>
        <div

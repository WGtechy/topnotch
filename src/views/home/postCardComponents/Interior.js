...
const Interior = (props) => {
  ...
  const arrData = useArrayData(currentData); 
   const {interiors,loading  } = useSelector((state) => state.posts);
  ...
  useEffect(() => {
    if (!!interiors?.length) {
      setCurrentData((init) => [...init, ...interiors]);
    } 
  }, [interiors]);

  useEffect(() => {
    if (accountId) {
      dispatch(getPosts({ page: postPage, accountId, target: "Interior" }));
    } else {
      dispatch(getPosts({ page: postPage, target: "Interior" }));
    }
...
// setIsPlaying={setIsPlaying}
  ...
  const categories = [
    {display: app?.allowAutomobiles, title: "Interior"},

...
const Property = (props) => {
  ...
  const arrData = useArrayData(currentData); 
   const {properties,loading  } = useSelector((state) => state.posts);
  ...
  useEffect(() => {
    if (!!properties?.length) {
      setCurrentData((init) => [...init, ...properties]);
    } 
  }, [properties]);

  useEffect(() => {
    if (accountId) {
      dispatch(getPosts({ page: postPage, accountId, target: "Property" }));
    } else {
      dispatch(getPosts({ page: postPage, target: "Property" }));
    }

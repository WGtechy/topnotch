...
const Shortlet = (props) => {
  ...
  const arrData = useArrayData(currentData); 
   const {shortlets,loading  } = useSelector((state) => state.posts);
  ...
  useEffect(() => {
    if (!!shortlets?.length) {
      setCurrentData((init) => [...init, ...shortlets]);
    } 
  }, [shortlets]);

  useEffect(() => {
    if (accountId) {
      dispatch(getPosts({ page: postPage, accountId, target: "Shortlet" }));
    } else {
      dispatch(getPosts({ page: postPage, target: "Shortlet" }));
    }
